...
class RNNModel(object):
  ...
  def __init__(self, vocabularySize, config_param):
    ...
    with tf.device("/cpu:0"): #Tells Tensorflow what GPU to use specifically
      ...
      #inputs = tf.split(embeddingLookedUp, self.config.sequence_size, 1)
    ...
    #singleRNNCell = rnn.BasicLSTMCell(self.config.hidden_size)
    ...
    #Defining Logits
    hidden_layer_output, last_state = rnn.rnn.rnn(self.multilayerRNN, inputTensorsAsList, initial_state=self._initial_state)
