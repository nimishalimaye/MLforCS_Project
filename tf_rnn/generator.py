...
from final_project.tf_rnn.model import RNNModel
from final_project.tf_rnn.data_reader import DataReader
import numpy as np
...
def main(unused_args):
  ...
  bs_reader = DataReader(tf.flags.FLAGS.dp,5)
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    with tf.variable_scope("TrumpBSQuoteModel", reuse=None, initializer=initializer):
      training_model = RNNModel(bs_reader.vocabularySize,config_param=config)
    ...
    with tf.variable_scope("TrumpBSQuoteModel", reuse=True, initializer=initializer):
      ...
      prediction_model = RNNModel(bs_reader.vocabularySize, config_param=eval_config)
    ...
    for epochCount in range(config.total_max_epoch):
      ...
      current_model_state = [training_model.initial_state[0].eval(),training_model.initial_state[1].eval()]
...
def get_prediction(model, bs_Reader, session, total_tokens, output_tokens = [' ']):
  ...
  zs = model.multilayerRNN.zero_state(1, tf.float32)
  state = [zs[0].eval(),zs[1].eval()]
...
class HyperParameterConfig(object):
  init_scale = 0.1
  learning_rate = 0.002*5
  max_grad_norm = 5
  num_layers = 2
  sequence_size = int(50/2)
  batch_size = 50*2
  hidden_size = 128
  embeddingSize = 100
